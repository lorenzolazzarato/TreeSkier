...
public class CharacterController : MonoBehaviour
{
    ...
    [SerializeField]
    private float _MaxTimeToSwipe = 1;
    ...
    private void CheckSwipe(Vector2 start, Vector2 finish, float time)
    {
        ...
        if (Math.Abs(start.y - finish.y) > _VerticalDistanceForSwipe && Math.Abs(start.x - finish.x) < _HorizontalDistanceForSwipe && time < _MaxTimeToSwipe)
        {
